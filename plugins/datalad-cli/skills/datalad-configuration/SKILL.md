...
argument-hint: '[dump|get|set|unset] [section.key] [value] [--scope global|local|branch]'
user-invocable: true
...
The three scopes are `global`, `local`, and `branch`. There is **no `dataset` scope** —
the committed, travels-with-the-dataset scope is called `branch`.

| Scope | Where stored | Effect |
|-------|-------------|--------|
| `branch` | `.datalad/config` (committed) | Applies to all clones of this dataset |
| `local` | `.git/config` (not committed) | Applies only to this clone |
| `global` | `~/.gitconfig` | Applies to all repos for this user |

Use `branch` scope for settings that should travel with the dataset (e.g., annex
...
3. **Execute**:
   ...
   datalad configuration get --scope branch datalad.
   ```
   ...
   For `branch` scope: the change writes to `.datalad/config`, which must be committed.
...
| Key | Scope | Purpose |
|-----|-------|---------|
| `annex.backend` | branch | Hash backend for new files (`SHA256E` recommended) |
| `annex.largefiles` | branch | Pattern deciding what goes into annex vs. git |
...
- For `--scope branch` changes: always remind the user to `datalad save` after setting —
